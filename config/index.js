...
class SLAMBOX {
    ...
    async rimrafLocal() {
        ...
        return fsPromises.rmdir(path.resolve(__dirname, '..dist'), { recursive: true })
    }
    ...
    server() {
        ...
        browserSync({
            ...
            files: [
                "./dist/components-template/**/style.css",
                "./dist/*.html",
                "./dist/critical.css",
                "./dist/components-template/**/script.min.js",
                "./dist/js/app.min.js",
                "./dist/images/",
            ],
            middleware: [
                function(req, res, next) {
                    if (lightMode) {
                        if (req.originalUrl.match(/(?<=\/dist\/).*(?=.*\.html)/i) && !req.originalUrl.includes('ajax')) {
                            _this.currentPage = req.originalUrl.match(/(?<=\/dist\/).*(?=.*\.html)/i)[0];
                        }
                    };
                    next();
                },
    ...
    buildMainJs() {
        return buildJs({
            ...
            output: path.resolve(__dirname, '../dist/js/app.min.js'),
            init: true
        });
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            glob("src/components/**/script.js", function(er, files) {
                files.forEach(src => {
                    ...
                    const output = path.resolve(__dirname, '../dist/components-template/', src.replace('src/components/', '').replace('script.js', 'script.min.js'));
                    promises.push(
                        buildJs({
                            input,
                            output,
    ...
    jsWatch() {
        ...
        (function() {
            ...
            watcher.on('change', link => {
                ...
                const output = path.resolve(__dirname, `../dist/components-template/${link.replace('src/components/', '').replace('script.js', 'script.min.js')}`);
                buildJs({
                    input,
                    output
                })

...
pagesGlob.forEach(page => {
	...
	entry.forEach(script => {
		const pathToScript = path.resolve(__dirname, `../dist/${script}`);
	...
	fs.writeFileSync(`${folder}/combinedJS/${page.replace('dist/', '')}.js`, scripts.join(''))
	html = fs.readFileSync(page, 'utf-8');
	const replaceItem = html.match(/<script data="component"><\/script>/);
	const newHtml = html.replace(replaceItem, `<script src="./combinedJS/${page.replace('dist/', '')}.js" defer></script>`);
	fs.writeFileSync(page, newHtml);
	scripts = [];
})
